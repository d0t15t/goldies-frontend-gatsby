...
export const CartItem: FC<CartItemProps> = (props) => {
  ...
  const getSubTitle = () => U.variantHasTitle(variantTitle) 
    ? quantity > 1 ? `${variantTitle}, ${eachPrice}` : variantTitle
    : eachPrice;
  
  return (
    <S.ListItem className={classes}>
      <Typography variant="h6" className={cls(['cart-item--title'])}>{title}</Typography>
      ...
      */}
       {
        cartItemsCount > 1 &&
         (<Box className={cls(['cart-item--total'])}>
          <Typography variant="span" className={cls(['cart-item--total-arrow'])}>☛</Typography><Price value={price * quantity} />
        </Box>)
       }
